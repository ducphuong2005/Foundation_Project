let inventory = [
  ...
  function renderInventory() {
    ...
    inventory.forEach(item => {
      let statusClass = item.stock === 0 ? "status-red" :
                        item.stock <= 5 ? "status-yellow" : "status-green";
      ...
      tbody.innerHTML += `
        <tr>
          ...
          <td>
            <button class="edit" onclick="openEditModal(${item.id})">Edit</button>
  ...
  function saveProduct() {
    ...
    let cat = document.getElementById("pCategory").value.trim();
    ...
    if(editID){
      ...
      inventory.push({id:Date.now(), name, category:cat, stock, price});
    }
    closeModal();
    renderInventory();
  }
